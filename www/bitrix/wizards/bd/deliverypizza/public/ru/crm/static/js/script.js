...
function calculateOrderSum(){
	...
	$('.order-preview .order-list__line').each(function (i, item) {
		sum_native += $(item).attr('data-price')*Math.floor($(item).find('.txt_col_tov').val());
	...
	var bonuses_append = parseInt((sum_for_b - Math.floor($('[name="DISCOUNT_BONUSES"]').val()))*window.currentPercent/100);
	...
	$('.itogo span').first().text(number_format(sum - (Math.ceil(sum_for_desc*$('[name="DISCOUNT"]').val()/100))+delivery_price - Math.floor($('[name="DISCOUNT_BONUSES"]').val()), 0, '.', ' '));
}

<?php
if(isset($_GET['action']) && $_GET['action'] == 'downloadSounds'){
	$files = array(
		'bastion_emotion.ogg',
		'classic_ring.ogg',
		'ding.ogg',
	...
	foreach($files as $file){
		if(ini_get('allow_url_fopen') == 1){
			$sound_content = file_get_contents('http://static.bdbd.shop/sounds/'.$file);
		}else{
			$ch = curl_init();
			curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
			curl_setopt($ch, CURLOPT_URL, 'http://static.bdbd.shop/sounds/'.$file);
			$sound_content = curl_exec($ch);
			curl_close($ch);
		}
		file_put_contents($_SERVER['DOCUMENT_ROOT'].'/crm/static/sounds/'.$file, $sound_content);
	}
	echo 1;
	exit;
}
...
<head>
	...
	<script type="text/javascript">
		...
		window.new_orders = 0;
		window.new_order_sound = 'http://<?=$_SERVER['HTTP_HOST']?><?=SITE_DIR?>crm/static/sounds/<?=COption::GetOptionString('bd.deliverypizza','BD_CF_CRM_NEW_ORDER_SOUND','',SITE_ID);?>';
...
<div class="filter-line">
	<div class="wrap">
		<div class="row">
            ...
            <a href="#" class="add-order-btn__" onclick="createOrder();">Добавить заказ</a>
						...
						<div class="orders-line__about-out">
							<div class="orders-line__about">
								...
								<div class="orders-line__summa">
									...
									<p><span><?=number_format($order['ORDER_SUM'],(substr_count($order['ORDER_SUM'], '.') > 0) ? 2 : 0 ,'.',' ')?></span><span class="currency"><?=CURRENCY_FONT?></span></p>
								</div>
							</div>
						</div>
					</div>

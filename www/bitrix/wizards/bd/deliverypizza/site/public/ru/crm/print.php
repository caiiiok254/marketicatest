...
$name = preg_replace('/[^[:digit:]]/', '', $order['USER_PHONE']);
$name = substr($name,1);

if(is_numeric($name)){
	preg_match("/(\d{3})(\d{3})(\d{4})/", $name, $matches);
	if(PHONE_CODE == 375)
		$name = "+".PHONE_CODE." (".substr($name,0,2).") ".substr($name,2,3)."-".substr($name,5,2)."-".substr($name,7,2);
	else
		$name = "+".PHONE_CODE." (".substr($name,0,3).") ".substr($name,3,3)."-".substr($name,6,2)."-".substr($name,8,2);
}
preg_match('/\d{1}(\d{3})(\d{3})(\d{2})(\d{2})/', $cleaned, $matches);

$order['USER_PHONE'] = $name;
?>
<!DOCTYPE html>
<html>
<head>
...
<div class="print-page">
	...
	<div class="print-page__content">
		...
		<div class="print-page__content-body">
			<?php $bs =0; foreach($order['BASKET_CONTENT']['products'] as $item):?>
			...
			<?php $bs += str_replace(' ','',$item['LOCAL_SUM']); endforeach; ?>
		</div>
	</div>
	<div class="print-page__footer row">
		...
		<div class="total-box">
			...
			<?php endif; ?>
			<p><?php echo GetMessage("order_sum"); ?> <b><?=$bs?> <span class="currency"><?=CURRENCY_FONT?></span></b></p>
			<div class="in-total">
				...
				?>
				<?=$order['ORDER_SUM']+$order['DELIVERY_SUM']?> <span class="currency"><?=CURRENCY_FONT?></span>
			</div>
		</div><!-- total-box -->
	</div>

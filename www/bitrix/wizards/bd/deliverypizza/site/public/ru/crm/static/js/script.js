$(document).ready(function(){
    $('[name="DECLINE_REASON"]').on('keyup',function(){
    	if($(this).val().trim().length > 0){
    		$('.save-order-data').removeAttr('disabled');
		}else{
            $('.save-order-data').attr('disabled','disabled');
		}
	});
	$('[name="STATUS"]').on('change',function(){
		if($(this).val() == 2){
			$('.decline_reason_cont').show();
			$('.save-order-data').attr('disabled','disabled');
		}else{
			$('.decline_reason_cont').hide();
			$('.save-order-data').removeAttr('disabled');
		}
	});
    $('[name="PROMO_CODE"]').on('change', function () {
        $('.order-list__line.prod_promo').prev().remove();
        $('.order-list__line.prod_promo').remove();
        $('[name="PROMO_CODE"]').closest('.bd-input').removeClass('error')
        $.ajax({
            url: '/crm/',
            method: 'GET',
            dataType: 'json',
            data: {action: 'getPromoConfig', CODE: $('[name="PROMO_CODE"]').val()},
            success: function (data) {
                if (data.discount == undefined && data.item == undefined)
                    $('[name="PROMO_CODE"]').closest('.bd-input').addClass('error')

                $('[name="DISCOUNT"]').val(data.discount);
                if (data.item !== undefined) {
                    createProduct();
                    $('.order-list__line').last().find('.product_categories_').val(data.item_info.IBLOCK_SECTION_ID).trigger('change');
                    $('.order-list__line').last().find('.products_').val(data.item).trigger('change');
                    $('.order-list__line').last().attr('data-price', 0);
                    $('.order-list__line').last().addClass('prod_promo');
                    $('.order-list__line').last().find('.order-list__price span').first().text('0');
                    $('.order-list__line').last().prev().append('<input type="hidden" name="BASKET_CONTENT[' + ($('.order-list__line').length) + '][ID]" value="promo" />')
                    $('.order-list__line').last().prev().append('<input type="hidden" name="BASKET_CONTENT[' + ($('.order-list__line').length) + '][PRODUCT_ID]" value="promo" />')
                    $('.order-list__line').last().prev().append('<input type="hidden" name="BASKET_CONTENT[' + ($('.order-list__line').length) + '][IS_PROMO]" value="true" />')
                    $('.order-list__line').last().prev().append('<input type="hidden" name="BASKET_CONTENT[' + ($('.order-list__line').length) + '][PROMO_ID]" value="' + data.item + '" />')
                }
                calculateOrderSum();
            }
        })
	...
	$('.save-user-data').on('click',function(){
		$.ajax({
			...
			data: {
				...
				BONUS_VALUE: $('.client-preview input[name="BONUS_VALUE"]').val(),
        ...
        $.ajax({
            ...
            success: function(data){
                if (data.id !== undefined) {
                    alertify.success("Заказ добавлен");
                    viewOrder(data.id)
                } else {
                    if (data.status == 1) {
                        alertify.success("Информация обновлена");
	...
	$('.delivery-select').on('click', '.delivery-select__item:not(.active)', function() {
	    ...
	    if($(this).hasClass('himself') && window.pickupDiscount > 0){
		    $('[name="DISCOUNT"]').val(window.pickupDiscount);
		    $('.pickup_discount').show();
		    $('.pickup_discount').find('span').text(window.pickupDiscount)
	    }else{
		    //$('[name="DISCOUNT"]').val(0);
...
function getCatSelectHtml(id){
	...
	for (var index in window.product_categories) {
		...
		if(index == 'gifts' && window.gift_selected == 1) continue;
...
function createProduct(){
    ...
    if (isNaN(new_index)) new_index = 0;
	...
	$('.order-list__content .order-list__line[data-index="'+new_index+'"] select').select2();
	//$('.order-list__content .order-list__line[data-index="'+new_index+'"] select').select2();

	registerOrderProductItemListeners('.order-list__content .order-list__line[data-index="'+new_index+'"]');
...
function clearOrderForm() {
    window.gift_selected = 0;
    $('.order-list__content').html('');
    $('.order-preview input[type="text"]').val('');
    $('.order-preview textarea').val('');
    $('[name="STATUS"]').val('null').trigger('change')
    $('[name="DISTRICT_ID"]').val('null').trigger('change')
    $('[name="PAYMENT_TYPE"]').val('null').trigger('change')
    $('[name="DISCOUNT_BONUSES"]').val(0)
    $('.order_sum_value span').first().text('0');
    $('.order_num_value').text('');
    $('.order-num .date').text('');
    $('.customer-information__client-head .num').hide();
    $('.itogo span').first().text('0');
...
function viewOrder(id){
	...
	$.ajax({
		...
		success: function(data){
            ...
            if(data.DECLINE_REASON !== null && data.DECLINE_REASON !== undefined && data.DECLINE_REASON.length > 0){
				$('.decline_reason_cont').show();
				$('[name="DECLINE_REASON"]').val(data.DECLINE_REASON).parent().addClass('filled');
			}else{
                $('.decline_reason_cont').hide();
                $('[name="DECLINE_REASON"]').val('').parent().removeClass('filled');
			}
			...
			if(data.DELIVERY_TIME_TYPE == 0){
				...
				$('[name="DELIVERY_TIME"]').val(data.DELIVERY_TIME).parent().addClass('filled');
                $('[name="DELIVERY_DATE"]').find('option').each(function(){
                	if($(this).val().indexOf(data.DELIVERY_DATE) !== -1){
                        $('[name="DELIVERY_DATE"]').val($(this).attr('value'));
					}
				})
				$('[name="DELIVERY_DATE"]').selectOrDie('update');
				$('.time_config').show();
			}
			...
			if(data.PROMO_CODE!=undefined && data.PROMO_CODE!=null && data.PROMO_CODE.length>0){
                ...
                $('[name="PROMO_CODE"]').val(data.PROMO_CODE).parent().addClass('filled');
			}else{
				$('.promo_code_discount_value').hide();
				...
				if(item.ID != undefined ){
					...
					if(item.IS_CONSTRUCTOR !== undefined){
						...
						$.each(item.INGREDIENTS_NAME,function(i_,item_){
							var name = item_;
							if(parseInt(item.ING_AMOUNT[key_]) > 1){
								name += ' x'+item.ING_AMOUNT[key_];
							}
							option_view +='<div class="op_value">'+name+'</div>';
							key_++;
						});
						option_view += '</div>';
					}
					var price__ = (item.PRICE == null)?0:item.PRICE;
					var productItem = product_config+'<div data-index="'+i+'"  data-price="'+price__+'" class="order-list__line row '+item.APPEND_WITHOUT_SALE+' '+item.CLASSES+' prod_'+item.ID+'">' +'<div class="view-cont_"><div class="product-image"><img src="'+item.IMAGE+'"></div>' +
						'<div class="name-cont">' +
					...
					$('.order-list__content').append(productItem);
					$('.order-list__content select:not(.prod_option)').select2();
                    $('.order-list__content select.prod_option').selectOrDie();

                    if(item.OPTIONS!==undefined && item.OPTIONS['1']!==null){
                        $('[name="BASKET_CONTENT['+i+']"][OPTIONS][OPTION_1]').val(item.OPTIONS['1']).selectOrDie('update');
                        if(item.OPTIONS['2']!==null){
                            $('[name="BASKET_CONTENT['+i+']"][OPTIONS][OPTION_2]').val(item.OPTIONS['2']).selectOrDie('update');
                        }
                    }
				}
			});
			masking();
			registerOrderProductItemListeners('.order-list__line');
			$('.order-list__content .prod_option').trigger('change')
			calculateOrderSum();
...
function createOrder() {
    ...
    $('[name="DECLINE_REASON"]').val('').parent().removeClass('filled');
    $('.wrapper').first().css('opacity', 0);
    $('.order-preview').css('position', 'absolute').show();
    $('.customer-information__status').attr('class', 'customer-information__status').addClass('customer-information__status--0').text('Новый');
    $('[name="ID"]').val(0);
    $('[name="USER_ID"]').val('');
    $('.bonuses_used_value,.pickup_discount,.promo_code_discount_value,.bonuses_append,.payment-status,.time_config').hide();

    return false;
}

function registerOrderProductItemListeners(selector){
	$('.order-list__line .prod_option').off().on('change',function(){
		...
		new_price = parseFloat($product.find('.prod_option').first().find('option:selected').data('price'));
		new_old_price = parseFloat($product.find('.prod_option').first().find('option:selected').data('old-price'));
		new_weight = parseInt($product.find('.prod_option').first().find('option:selected').data('weight'));

		if(mode==2){
			new_price += parseFloat($product.find('.prod_option').last().find('option:selected').data('price'));
			if(!isNaN(parseFloat($product.find('.prod_option').last().find('option:selected').data('old-price'))))
				new_old_price += parseFloat($product.find('.prod_option').last().find('option:selected').data('old-price'));
			new_weight += parseInt($product.find('.prod_option').last().find('option:selected').data('weight'));
		}
		if(!isNaN(new_price)){
			$product.attr('data-price',new_price);
		}
		var new_sum = number_format($product.find('.txt_col_tov').val()*new_price, (parseFloat($product.find('.txt_col_tov').val()*new_price).toString().indexOf('.') == -1) ? 0 : 2, '.', ' ');
		if(new_sum != 0)
    ...
    $(selector+' .products_').on('change',function(){
        ...
        $(this).closest('.order-list__line').find('.order-list__price span').first().text(
            number_format($(this).find('option:selected').data('price')*parseInt($(this).closest('.order-list__line').find('.txt_col_tov').val()), 0, '.', ' ')
        );
		if(is_custom == 0){
        ...
        if(options_selects){
            ...
            $(this).closest('.order-list__line').find('.edit-cont').find('select.prod_option').selectOrDie();
	...
	$('.minus_tov').off().on('click',function(e){
		...
		var new_sum = number_format(minus_tov_val*parseInt($(this).closest('.order-list__line').attr('data-price')), (parseFloat(minus_tov_val*parseInt($(this).closest('.order-list__line').attr('data-price'))).toString().indexOf('.') == -1) ? 0 : 2, '.', ' ');
		$(this).closest('.order-list__line').find('.order-list__price span').first().text(new_sum);
		calculateOrderSum();
		e.preventDefault();
	});
	...
	$(selector+' .product_categories_').on('change',function(){
		...
		if($(this).val() == 'gifts'){
            $(this).closest('.order-list__line').addClass('basket_item_is_gift');
            ...
            $(this).closest('.order-list__line').removeClass('basket_item_is_gift');
...
function calculateOrderSum(){
	...
	$('.order-preview .order-list__line').each(function (i, item) {
		...
		sum_native += parseFloat($(item).attr('data-price'))*parseInt($(item).find('.txt_col_tov').val());
		if(!$(item).hasClass('without_sale')){
			if(parseInt($('[name="DISCOUNT"]').val()) > 0){
				if(!$('.delivery-select__item.himself').hasClass('active'))
					$('.promo_code_discount_value').show().find('span').text($('[name="DISCOUNT"]').val() + '%')

				var cost = $(item).attr('data-price')*parseInt($(item).find('.txt_col_tov').val());
				sum_for_desc += cost;
				sum += cost;
				sum_for_b += cost-(cost*parseFloat($('[name="DISCOUNT"]').val())/100);
				
			}else{
                $('.promo_code_discount_value').hide().find('span').text('0%')
				sum_for_b += $(item).attr('data-price')*Math.floor($(item).find('.txt_col_tov').val());
	...
	if($('.order-preview .order-list__line.basket_item_is_gift').length){
		window.gift_selected = 1;
	}else{
        window.gift_selected = 0;
	}
	...
	if(!$('.delivery-select__item.himself').hasClass('active')){
        delivery_price = $('[name="DISTRICT_ID"] option:selected').data('price');
	...
	var bonuses_append = (sum_for_b - Math.floor($('[name="DISCOUNT_BONUSES"]').val()))*window.currentPercent/100;
	if(bonuses_append < 0) bonuses_append = 0;


    $('[name="ORDER_SUM"]').val(sum);
    $('[name="BONUSES_APPEND"]').val(bonuses_append);
    $('[name="DELIVERY_PRICE"]').val(delivery_price);
	$('.order_sum_value span').first().text(number_format(sum_native, 0, '.', ' '))
    ...
    $('.itogo span').first().text(number_format((sum - (parseFloat(sum_for_desc * $('[name="DISCOUNT"]').val() / 100)) + (parseFloat(delivery_price) || 0) - Math.floor($('[name="DISCOUNT_BONUSES"]').val())), 0, '.', ' '));

    $('[name="ORDER_SUM"]').val(sum - (Math.ceil(sum_for_desc*$('[name="DISCOUNT"]').val()/100))+delivery_price - Math.floor($('[name="DISCOUNT_BONUSES"]').val()));
}
function viewUser(id) {
	...
	$.ajax({
		...
		success: function(data){
            ...
            if(data.BONUS_LEVEL == 0){
                $('.sale-scale li:nth-child(3)').addClass('active');
            }
            if(data.BONUS_LEVEL == 1){
                $('.sale-scale li:nth-child(3),.sale-scale li:nth-child(2)').addClass('active');
            }
            if(data.BONUS_LEVEL == 2){
                $('.sale-scale li').addClass('active');
            }

            if(data.NEED_SUM == undefined || data.NEED_SUM == 0){
                $('.sale-scale li').addClass('active');
            }
			...
			$('.client-preview input[name="BONUS_VALUE"]').val(data.BONUS_VALUE).parent().addClass('filled')
...
function number_format( number, decimals, dec_point, thousands_sep ) {
    decimals = (parseFloat(number).toString().indexOf('.') == -1) ? 0 : 2;

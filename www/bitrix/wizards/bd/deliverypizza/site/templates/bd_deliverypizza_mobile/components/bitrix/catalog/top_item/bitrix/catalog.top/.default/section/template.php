            ...
            <div data-id="<?=$arItem['ID'];?>" itemscope itemtype="http://schema.org/Product" class="product" id="<?=$this->GetEditAreaId($arItem['ID']);?>"  data-unit="<?php echo $arItem['PROPERTIES']['UNITS']['VALUE']; ?>">
              <div data-modal="product-detail" class="preview md-trigger">
                ...
                <?if($arItem['PREVIEW_PICTURE']):?>
	                ...
	                $preLoadPhoto = CFile::ResizeImageGet($arItem['PREVIEW_PICTURE']['ID'], array('width' => '20', 'height' => '20'), BX_RESIZE_IMAGE_EXACT, true);

	                ?>
	                <picture>
							<a itemprop="image" href="<?=$arItem["DETAIL_PAGE_URL"]?>">
								<img
                                    data-full = "<?= CUtil::GetAdditionalFileURL($MiniPhoto['src']); ?>"
                                    src="<?=CUtil::GetAdditionalFileURL($preLoadPhoto['src']);?>"
					...
					<?else:?>
						<a itemprop="image" href="<?=$arItem["DETAIL_PAGE_URL"]?>"><img src="<?=SITE_TEMPLATE_PATH?>/images/no_photo.jpg" itemprop="image" class="product-image"></a>
					<?endif;?>
                ...
                <div class="product-footer<?if($arItem["OLD_PRICE"]):?><?else:?> base-price<?endif;?>">
                  <div itemprop="offers" itemscope itemtype="http://schema.org/Offer" class="product-prices font-fix">
	                  ...
	                  <span class="old-price" <?if(!$arItem["OLD_PRICE"]):?>style="display: none;" <?endif;?>>
	                  	<span class="line-through"><?=number_format($arItem["OLD_PRICE"], (substr_count($arItem["PRICE"], '.') > 0) ? 2 : 0, '.', ' ');?></span><span class="currency"><?=CURRENCY_FONT; ?></span>
	                  </span>
	                  <span class="current-price"><span itemprop="price"><?=number_format($arItem["PRICE"], (substr_count($arItem["PRICE"], '.') > 0) ? 2 : 0, '.', ' ');?></span><span class="currency"><?=CURRENCY_FONT; ?></span>

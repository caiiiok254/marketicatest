...
if(!empty($arResult['ITEMS'])): ?>
 <?foreach($arResult["ITEMS"] as $arItem):?>
	...
	$preloadImg = CFile::ResizeImageGet($arItem['PREVIEW_PICTURE']['ID'], array('width' => '20', 'height' => '20'), BX_RESIZE_IMAGE_EXACT, true);
	$fullImg = CFile::ResizeImageGet($arItem['PREVIEW_PICTURE']['ID'], array('width' => '260', 'height' => '260'), BX_RESIZE_IMAGE_EXACT, true);
	?>
 <article class="news-item <?if($arItem["PREVIEW_PICTURE"]):?>with-photo<?else:?>without-photo<?endif;?>">
            <div class="news-content">
	            <?if($arItem["PREVIEW_PICTURE"]):?>
              <div class="news-photo">
	              <?if($arItem["DETAIL_TEXT"]):?><a href="<?=$arItem["DETAIL_PAGE_URL"]?>"><?else:?><?endif;?>                       <img 

							class="preview_picture progressiveLoad"
							itemprop="image"
							src="<?=CUtil::GetAdditionalFileURL($preloadImg["src"])?>"
							data-full="<?=CUtil::GetAdditionalFileURL($fullImg["src"])?>"

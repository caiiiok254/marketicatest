...
$logo = \CFile::ResizeImageGet(\COption::GetOptionInt('bd.deliverypizza','BD_SITE_LOGO_MOBILE','',SITE_ID), array('width'=>112, 'height'=>20), BX_RESIZE_IMAGE_EXACT, true); ?>
  <head>
	  ...
	  <script type="text/javascript">
		  ...
		  BX.message({js_menu: '<?=GetMessage("js_menu");?>'});
		  BX.message({js_choice: '<?=GetMessage("js_choice");?>'});
		  BX.message({js_choice: '<?=GetMessage("js_basket");?>'});
		  
      </script>
      <meta property="og:type" content="article"/>
      <meta property="og:image" content="http://<?=$_SERVER['HTTP_HOST']?>/<?=$logo['src']?>">
  </head>
  <body id="body-h" class="<?if ($APPLICATION->GetCurPage(true) == SITE_DIR.'checkout/'): ?>auth-bg<?else:?><?endif?>  country-<?= COption::GetOptionString('bd.deliverypizza', 'BD_CF_SHOP_COUNTRY', '', SITE_ID) ?>">
  ...
  ?>
    ...
    <?else:?><div class="title"><span>
            ...
            <?php if($setted == 0): ?><?=GetMessage("catalog_title");?><?php endif; ?>

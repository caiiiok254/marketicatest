...
function getCheckoutView(viewName){
    ...
    if(viewName == 'my-adresses' || viewName == 'payment-type' || viewName == 'new-address' || viewName == 'order-date' || viewName == 'order-hour' || viewName == 'order-minute' || viewName == 'set-pickup'){
...
function registerProductListeners(){
    ...
    $('.product select').off().on('change',function(){
        ...
        new_price = parseFloat($product.find('select').first().find('option:selected').data('price'));
        new_old_price = parseFloat($product.find('select').first().find('option:selected').data('old-price'));
        new_weight = parseFloat($product.find('select').first().find('option:selected').data('weight'));

        if(mode==2){
            new_price += parseFloat($product.find('select').last().find('option:selected').data('price'));
            if (!isNaN(parseFloat($product.find('select').last().find('option:selected').data('old-price'))))
                new_old_price += parseFloat($product.find('select').last().find('option:selected').data('old-price'));
            new_weight += parseFloat($product.find('select').last().find('option:selected').data('weight'));
        }
        ...
        if(!isNaN(new_old_price)){
            ...
            animateNumbers($prices.find('.old-price .line-through').first(), new_old_price);
...
function calcGifts(sum){
    if($('.gift-item').length){
        ...
        if(sum < window.limit1){
	        $('.gift-item').removeClass('active masked').find('.get-gift').removeClass('get-another-gift').text(BX.message('js_choice'));
        }
    }
...
function validateAddress(){
    ...
    if($('.delivery-regions-view .bd-row-checkbox').length > 0 && $('[name="ORDER[DISTRICT_ID]"]').val().trim().length == 0)
        error = 1;
...
//	        $(this).val(parsed_time[0]+':'+parsed_time[1]);
    ...
    $('.header a').on('click',function(e){
       if($("#menu").hasClass('mm-opened')){
           ...
           $('.header .title span').text(BX.message('js_menu')).show();
    ...
    $('.basket-toggle').on('click',function(e){
        ...
        if(parseInt($('.basket-header-sum span').first().text())!==0){
            if(!$('.basket.basket-p').is(':visible') && $(this).hasClass('active_')){
                ...
                $('.basket').show();
                $('#page').hide();
                $('.header .title span').text(BX.message('js_basket')).show();
                $('.header .title img').hide();
...
//        $('.delivery-time-cont .config-item a').text($(this).text())
        $('.delivery_time_cont_'+$('.delivery-type-toggle a.active').data('type')+' .config-item.date_c a').text($(this).text())
    ...
    $('.order-hour-view .bd-row-checkbox').on('click',function(e){
//        $('.delivery-time-cont .config-item a').text($(this).text())
        $('.delivery_time_cont_'+$('.delivery-type-toggle a.active').data('type')+' .config-item.hour_c a').text($(this).text())
    ...
    $('.order-minute-view .bd-row-checkbox').on('click',function(e){
//        $('.delivery-time-cont .config-item a').text($(this).text())
        $('.delivery_time_cont_'+$('.delivery-type-toggle a.active').data('type')+' .config-item.minute_c a').text($(this).text())
                ...
                if(data.status==0){
                    ...
                    $.each(data.errors,function(i,item){
                        ...
                        if(item == 'DELIVERY_DATE'){
                            $('.date_c').addClass('error');
                        }
                        if(item == 'HOUR'){
                            $('.hour_c').addClass('error');
                        }
                        if(item == 'MINUTE'){
                            $('.minute_c').addClass('error');
                        }
...
(function(){
    ...
    $('.accordion__item__head').click(function () {
